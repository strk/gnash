// ProgressEvent_as3.cpp:  ActionScript "ProgressEvent" class, for Gnash.
//
...
#include "events/ProgressEvent_as3.h"
#include "log.h"
#include "fn_call.h"
...
void progressevent_class_init(as_object& global)
{
    ...
    if (!cl) {
        cl = new builtin_function(&progressevent_ctor, getProgressEventInterface());
...
{
    boost::intrusive_ptr<ProgressEvent_as3> ptr =
        ensureType<ProgressEvent_as3>(fn.this_ptr);
    UNUSED(ptr);
...
{
    boost::intrusive_ptr<as_object> obj = new ProgressEvent_as3;
