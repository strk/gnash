// TextEvent_as3.cpp:  ActionScript "TextEvent" class, for Gnash.
//
...
#include "events/TextEvent_as3.h"
#include "log.h"
#include "fn_call.h"
...
void textevent_class_init(as_object& global)
{
    ...
    if (!cl) {
        cl = new builtin_function(&textevent_ctor, getTextEventInterface());
...
{
    boost::intrusive_ptr<TextEvent_as3> ptr =
        ensureType<TextEvent_as3>(fn.this_ptr);
    UNUSED(ptr);
...
{
    boost::intrusive_ptr<as_object> obj = new TextEvent_as3;
