// SimpleButton_as.cpp:  ActionScript "SimpleButton" class, for Gnash.
//
...
namespace {
    as_value simplebutton_ctor(const fn_call& fn);
    void attachSimpleButtonInterface(as_object& o);
    void attachSimpleButtonStaticInterface(as_object& o);
...
void simplebutton_class_init(as_object& global)
{
    ...
    if (!cl) {
        cl = new builtin_function(&simplebutton_ctor, getSimpleButtonInterface());
        attachSimpleButtonStaticInterface(*cl);
    }

    // Register _global.SimpleButton
    global.init_member("SimpleButton", cl.get());
}
