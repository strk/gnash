...
//		        string_table::key methodKey = tdata->st->find(methodName);
	...
	boost::shared_ptr<amf::Buffer> buf(new amf::Buffer(300000));
	do {
...
// 		    done = true;
	...
	if (result) {
	    ...
	    http->dump();
#if 0
	    // FIXME: reconnect if needbe
	    if (http->getStatus() >= HTTP::MULTIPLE_CHOICES) &&
		(http->getStatus() >= HTTP::TEMPORARY_REDIRECT) {
		    // do something here
		    string location = http->getField("Location");
		}
#endif
	    string location = http->getField("Location");
	    if (!location.empty()) {
		URL url(location);
		short port = strtol(url.port().c_str(), NULL, 0) & 0xffff;
		if (!args->network->createClient(url.hostname(), port)) {
		    log_error("Can't connect to server %s on port %hd",
			      url.hostname(), port);
		}
	    }
