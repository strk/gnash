// IDynamicPropertyOutput_as3.cpp:  ActionScript "IDynamicPropertyOutput" class, for Gnash.
//
...
#include "net/IDynamicPropertyOutput_as3.h"
#include "log.h"
#include "fn_call.h"
...
void idynamicpropertyoutput_class_init(as_object& global)
{
    ...
    if (!cl) {
        cl = new builtin_function(&idynamicpropertyoutput_ctor, getIDynamicPropertyOutputInterface());
        attachIDynamicPropertyOutputStaticInterface(*cl);
    }

    // Register _global.IDynamicPropertyOutput
    global.init_member("IDynamicPropertyOutput", cl.get());
}
...
{
    boost::intrusive_ptr<as_object> obj = new IDynamicPropertyOutput_as3;
