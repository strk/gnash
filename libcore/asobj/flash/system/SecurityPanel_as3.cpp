// SecurityPanel_as3.cpp:  ActionScript "SecurityPanel" class, for Gnash.
//
...
#include "system/SecurityPanel_as3.h"
#include "log.h"
#include "fn_call.h"
...
void securitypanel_class_init(as_object& global)
{
    ...
    if (!cl) {
        cl = new builtin_function(&securitypanel_ctor, getSecurityPanelInterface());
        attachSecurityPanelStaticInterface(*cl);
    }

    // Register _global.SecurityPanel
    global.init_member("SecurityPanel", cl.get());
}
...
{
    boost::intrusive_ptr<as_object> obj = new SecurityPanel_as3;
