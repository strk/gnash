...
NetStream_as::play(const std::string& c_url)
{
	...
	{
		IF_VERBOSE_ASCODING_ERRORS(
		log_aserror(_("No NetConnection associated with this NetStream_as, won't play"));
		);
		return;
	}
...
NetStream_as::initVideoDecoder(media::MediaParser& parser)
{
	...
	if (!videoInfo) {
		log_debug("No video in NetStream_as stream");
		return;
	}
    ...
    try {
	    ...
	    log_error("NetStream_as: Could not create Video decoder: %s", e.what());
	}
...
NetStream_as::initAudioDecoder(media::MediaParser& parser)
{
	...
	if (!audioInfo) {
		log_debug("No audio in NetStream_as input");
		return;
	}
...
NetStream_as::startPlayback()
{
	...
	if ( ! m_parser.get() )
	{
		log_error(_("Unable to create parser for NetStream_as input"));
...
NetStream_as::decodeNextVideoFrame()
{
	...
	if ( ! video.get() )
	{
		...
		log_error(_("Error decoding encoded video frame in NetStream_as input"));
	}
...
NetStream_as::attachAuxStreamer()
{
	...
	try {
		...
		log_error("Could not attach NetStream_as aux streamer to sound handler: %s", e.what());
	}
}
