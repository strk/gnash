...
public:
    ...
    typedef std::pair<std::string, int> Export;
    ...
    virtual void executeState(MovieClip* m, DisplayList& /*l*/) const {
        ...
        for (Exports::const_iterator it = _exports.begin(), e = _exports.end();
                it != e; ++it) {
            log_debug("Exporting %s", it->first);
            const_cast<movie_definition*>(mov->definition())->exportResource(it->first, it->second);
        }
    }
...
private:
    void read(SWFStream& in, movie_definition& m) {
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            _exports.push_back(std::make_pair(symbolName, id));
        }
