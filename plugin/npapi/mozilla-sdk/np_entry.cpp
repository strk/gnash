...
NPError
fillBrowserFunctionTable(NPNetscapeFuncs* aNPNFuncs)
{
    gnash::log_debug(__PRETTY_FUNCTION__);
...
NP_Initialize(NPNetscapeFuncs* aNPNFuncs)
{
    ...
    NPError rv = fillBrowserFunctionTable(aNPNFuncs);
#if 0
...
NPError
main(NPNetscapeFuncs* nsTable, NPPluginFuncs* pluginFuncs,
...
NPError
main(NPNetscapeFuncs* aNPNFuncs, NPPluginFuncs* aNPPFuncs,
             NPP_ShutdownUPP* aUnloadUpp)
{
    ...
    if (rv == NPERR_NO_ERROR) {
        rv = fillBrowserFunctionTable(aNPNFuncs);
    }
