...
{
public:
    PointSerializer(std::vector<boost::uint16_t>& dest)
        :
        _dest(dest)
    {}
...
private:
    std::vector<boost::uint16_t>& _dest;
};
...
# endif
                  ...
                  const SWFMatrix& mat)
  {
    ...
    std::vector<boost::uint16_t> pointList;
    pointList.reserve(coords.size() * 2);
    ...
    glVertexPointer(2, GL_SHORT, (sizeof(boost::int16_t) * 2) *
            (pointList.size() - 1), &pointList.front());
