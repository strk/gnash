// render_handler_xbox.cpp	-- Thatcher Ulrich <http://tulrich.com> 2003

// This source code has been donated to the Public Domain.  Do
// whatever you want with it.

// A gnash::render_handler that uses Xbox API's
	...
	// Initialize the vshader we use for SWF mesh rendering.
	{
		if (s_vshader_handle == 0)
		{
			...
			*/
			...
			if (result != S_OK)
			{
				gnash::log_error("error: can't create Xbox vshader; error code = %d\n", result);
				return;
			}
		}
	}
...
// Image with no alpha.
{
	...
	if (result != S_OK)
	{
		gnash::log_error("error: can't load surface from memory, result = %d\n", result);
		return;
	}
