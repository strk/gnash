...
#if 0 // non-blocking seek
	// Seek to next frame to index  (non-blocking)
	//log_debug("FLVParser::indexNextTag seeking to %d", thisTagPos);
	try {
		// Seek to next frame and skip the tag size 
		if ( _stream->seekNonBlocking(thisTagPos+4) )
		{
			//log_debug("FLVParser::indexNextTag: can't seek (non-blocking) to %d", thisTagPos+4);
			//_indexingCompleted=true;
			//_parsingComplete=true; // better let ::parseNextTag set this
			return false;
		}
		//log_debug("FLVParser::indexNextTag seeked to %d", thisTagPos+4);
	} catch (IOException& x) {
		log_debug("FLVParser::indexNextTag failed seeking to %d: %s", thisTagPos+4, x.what());
		_indexingCompleted=true;
		return false;
	}
#else
