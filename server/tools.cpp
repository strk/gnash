// tools.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2004
...
namespace tools {
	...
	public:
		...
		// Copy the data.  Return true on success, false on failure.
		{
			...
			{
				log_error("gnash::tools::copy_helper asked to copy %d bytes\n",
					  bytes_to_copy);
				return false;
			}
		}
...
int	gnash::tools::process_swf(tu_file* swf_out, tu_file* in, const process_options& options)
{
	...
	if (options.m_zip_whole_file)
	{
		...
		log_error("gnash::tools::process_swf(): options.m_zip_whole_file is not implemented!  Output will not be zipped.\n");
	}
	...
	int	frame_count = 0;
	{
		...
		{
			// Error!
			log_error("gnash::tools::process_swf() -- unable to copy header data!\n");
			return 1;
		}
	}
...
//	IF_VERBOSE_PARSE(m_frame_size.print());
	...
	while ((boost::uint32_t) str.get_position() < file_end_pos)
	{
		...
		else
		{
			...
			{
				// Error!
				log_error("gnash::tools::process_swf() -- error copying tag!\n");
				return 1;
			}
		}
